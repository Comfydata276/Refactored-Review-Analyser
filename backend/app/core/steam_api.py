...
class SteamAPI:
    ...
    def get_app_list(self):
        ...
        try:
            ...
            logger.info(
                f"Successfully fetched and cached {len(self._app_list_cache)} apps."
            )
    ...
    def fetch_reviews_for_app(
        ...
        reviews = []
        cursor = "*"
        review_language = self.config_manager.get_setting(
            ["fetching", "language"], "all"
        )
        request_count = 0
        ...
        max_consecutive_errors = (
            5  # NEW: Allow some consecutive errors before giving up
        )
        ...
        periodic_save_interval = self.config_manager.get_setting(
            ["analysis", "periodic_save_interval"], 100
        )

        # Track elapsed time for complete scraping
        start_time = datetime.now()
        # Enforce overall scraping timeout (seconds); default 1 hour if not set
        try:
            scraping_timeout = int(
                self.config_manager.get_setting(["fetching", "scraping_timeout"], 3600)
            )
        ...
        if scrape_all:
            ...
            try:
                num_reviews_to_fetch = int(
                    self.config_manager.get_setting(
                        ["fetching", "reviews_per_app"], 100
                    )
                )
                max_requests = int(
                    self.config_manager.get_setting(
                        ["fetching", "max_requests_per_app"], 5
                    )
                )
            except (ValueError, TypeError):
                logger.error(
                    "Invalid non-numeric value in fetching settings. Using defaults."
                )
        ...
        logger.info(
            f"🔍 DEBUG: fetch_reviews_for_app called with app_id={app_id}, scrape_all={scrape_all} (type: {type(scrape_all)})"
        )
        ...
        while len(reviews) < num_reviews_to_fetch and request_count < max_requests:
            ...
            if stop_event and stop_event.is_set():
                logger.info(
                    f"Stop signal received during scraping of app {app_id}. Stopping gracefully."
                )
                break

            # Enforce wall-clock timeout to prevent hanging scrapes
            elapsed_seconds = (datetime.now() - start_time).total_seconds()
            if scraping_timeout and elapsed_seconds > scraping_timeout:
                logger.info(
                    f"Scraping timeout reached ({scraping_timeout}s) for app {app_id}; stopping."
                )
                if progress_callback:
                    progress_callback(
                        {
                            "type": "log",
                            "message": f"Scraping timeout reached ({int(elapsed_seconds)}s) for app {app_id}",
                            "level": "warning",
                        }
                    )
                break
            ...
            if consecutive_empty_batches >= max_empty_batches:
                logger.info(
                    f"Stopping scrape for app {app_id} - {consecutive_empty_batches} consecutive empty batches"
                )
                break

            if consecutive_errors >= max_consecutive_errors:
                logger.warning(
                    f"Stopping scrape for app {app_id} - {consecutive_errors} consecutive errors"
                )
                break

            if total_errors >= max_total_errors:
                logger.warning(
                    f"Stopping scrape for app {app_id} - {total_errors} total errors reached"
                )
                break
            ...
            try:
                logger.debug(
                    f"Making request {request_count} for app {app_id} with cursor: {cursor}"
                )
                ...
                if response.status_code == 502:
                    raise requests.exceptions.HTTPError(
                        "502 Bad Gateway - Steam servers temporarily unavailable"
                    )
                elif response.status_code == 503:
                    raise requests.exceptions.HTTPError(
                        "503 Service Unavailable - Steam servers overloaded"
                    )
                ...
                if data.get("success") != 1:
                    logger.warning(
                        f"API returned success={data.get('success')} for app {app_id}"
                    )
                ...
                if not batch_reviews:
                    ...
                    logger.debug(
                        f"Empty batch {consecutive_empty_batches} for app {app_id}"
                    )
                    ...
                    if scrape_all and consecutive_empty_batches < max_empty_batches:
                        ...
                        logger.info(
                            f"No reviews found for app {app_id} in batch {request_count}"
                        )
                        break
                else:
                    consecutive_empty_batches = 0
                ...
                if (
                    scrape_all
                    and len(reviews) % periodic_save_interval == 0
                    and periodic_save_callback
                ):
                    periodic_save_callback(reviews.copy(), app_id)
                    logger.info(
                        f"Periodic save triggered for app {app_id} at {len(reviews)} reviews"
                    )
                ...
                if "502" in str(e) or "503" in str(e):
                    ...
                    logger.info(
                        f"Server error detected, waiting {wait_time} seconds before retry..."
                    )
                    time.sleep(wait_time)
                elif "429" in str(e):
                    wait_time = min(
                        60, 10 * consecutive_errors
                    )  # Up to 60 seconds for rate limiting
                    logger.info(
                ...
                total_errors += 1
                logger.error(
                    f"Request error fetching reviews for app {app_id} (request {request_count}): {e}"
                )
                time.sleep(5)
                continue  # Don't break, try again
        ...
        if scrape_all:
            msg = (
                f"Complete scrape finished for app {app_id}: "
                f"{final_count:,} reviews in {elapsed_str} (Total errors: {total_errors})"
            )
        else:
            msg = (
                f"Limited scrape finished for app {app_id}: "
        ...
        if progress_callback:
            progress_callback({"type": "log", "message": msg, "level": "info"})
